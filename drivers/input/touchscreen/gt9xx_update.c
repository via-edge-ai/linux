...
#define GOODIX_FIRMWARE_FILE_NAME	"touch.BIN"
				...
				struct st_fw_head *fw_head, u8 *path)
{
	...
	if(path){
		...
		if (IS_ERR(update_msg.file)) {
			dev_err(&client->dev,"Open update file(%s) error!", path);
			return FAIL;
		}
		update_msg.fs = get_fs();
		set_fs(KERNEL_DS);
		...
		/*ret = update_msg.file->f_op->read(update_msg.file,
			update_msg.fw_data, update_msg.fw_total_len, &update_msg.file->f_pos);*/
		ret = vfs_read(update_msg.file,
			(char __user*)update_msg.fw_data, update_msg.fw_total_len, &update_msg.file->f_pos);

		if (ret < 0) {
			dev_err(&client->dev,"Read firmware in update file error.");
			set_fs(update_msg.fs);
			filp_close(update_msg.file, NULL);
			/*_CLOSE_FILE(update_msg.file);*/
		}
		else if (update_msg.fs){
			set_fs(update_msg.fs);
			filp_close(update_msg.file, NULL);
		}
	}else{
