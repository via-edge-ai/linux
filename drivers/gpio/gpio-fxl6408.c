...
static void fxl6408_gpio_set_value(struct gpio_chip *gc, unsigned off, int val)
{
	...
	dev_info(dev, "FXL6408 set value(0x%02x, %d)\n", off, val);
			 ...
			 const struct i2c_device_id *id)
{
	...
	dev_info(dev, "FXL6408 Start probe\n");
	...
	/* Regulator */
	chip->supply = regulator_get_optional(dev, "power");
	if (IS_ERR(chip->supply)) {
		dev_err(dev, "Can't get regulator");
		if (PTR_ERR(chip->supply) == -ENODEV) {
			chip->supply = NULL;
		} else {
			dev_err(dev, "Can't get regulator: %ld\n",
				PTR_ERR(chip->supply));
		}
	} else {
		//Enable power for check id
		if (chip->supply) {
		...
		dev_err(dev, "Enable regulator done");
	}
	...
	while (retry > 0) {
		...
		if (device_id < 0) {
			...
			return device_id;
		} else if (device_id >> 1 & 0x1) {
			dev_info(dev, "FXL6408 is in reset");
			msleep(50);
			continue;
		} else if (device_id >> 5 != 5) {
			dev_info(dev, "FXL6408 probe returned DID: 0x%02x\n", device_id);
			continue;
		} else {
			dev_info(dev, "FXL6408 Get DID: 0x%02x\n", device_id);
			break;
		}
	}
	...
	if (of_property_read_u32(dev->of_node, "inital_io_dir", &val)) {
		...
		dev_err(dev, "chip->reg_io_dir: %x\n", chip->reg_io_dir);
		if (chip->reg_io_dir == 0) {
			chip->reg_io_dir = 0xff;
			i2c_smbus_write_byte_data(client, FXL6408_IO_DIR, chip->reg_io_dir);
		}

	} else {
		chip->reg_io_dir = val | 0xff;
		ret = i2c_smbus_write_byte_data(client, FXL6408_IO_DIR, chip->reg_io_dir);
		if (ret)
			dev_err(dev, "Failed to set io_dir: %d\n", ret);
	}
	...
	if (of_property_read_u32(dev->of_node, "inital_output", &val)) {
		...
		ret = i2c_smbus_write_byte_data(client, FXL6408_OUTPUT, chip->reg_output);
		if (ret)
			dev_err(dev, "Failed to set reg_output: %d\n", ret);
	}
	...
	dev_info(dev, "FXL6408 probe success(%s)\n", gc->label);
	return 0;
}
